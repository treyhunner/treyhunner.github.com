...
$(document).ready(function(){
  ...
  function submitHandler($form) {
    ...
    $.ajax({
      ...
      if (xhr.status == 201) {
        var thanksMessage = $('#morsels-signup-thanks').html();
        $title.html('You just signed up!... almost');
        $description.html(thanksMessage);
        if ($email.val().indexOf("gmail.com") != -1) {
          $description.find('.email-message').html('<p><a href="https://mail.google.com" target="_blank">Go to Gmail to continue</a> :)</p>')
        }
        gaSend({
          hitType: 'event',
          eventCategory: 'Python Morsels',
  ...
  $("#funnel-tools-capture form[action*=pythonmorsels]").submit(function(event){
